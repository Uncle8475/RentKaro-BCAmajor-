...
export const getCarModelData = async (req, res, next) => {
  try {
    let availableVehicleModels = await MasterData.find();

    // If no data exists, insert the dummy data
    if (!availableVehicleModels || availableVehicleModels.length === 0) {
      console.log("No model data found. Inserting dummy data...");
      await MasterData.insertMany(dummyData);
      availableVehicleModels = await MasterData.find();
    }
    ...
    next(errorHandler(500, { "could not get model Data": error.message }));
  }
};
