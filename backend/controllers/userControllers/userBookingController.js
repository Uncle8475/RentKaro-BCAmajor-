...
export const razorpayOrder = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        errorHandler(400, "Missing Required Fields Process Cancelled"),
      );
    }

    if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_SECRET) {
      console.error("Razorpay credentials missing");
      return next(errorHandler(500, "Razorpay credentials not configured"));
    }
    ...
    next(errorHandler(500, `Error creating razorpay order: ${error.message}`));
  }
};
